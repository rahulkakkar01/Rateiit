...
This API seeds fixed login accounts on every startup. These credentials stay consistent across restarts unless you change them in `.env`.
...
Default fixed accounts from `.env`:
...
For production, avoid fixed shared passwords and disable or restrict this seeding pattern.
